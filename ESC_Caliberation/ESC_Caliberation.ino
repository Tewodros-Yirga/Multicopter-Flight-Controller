...
void loop(){
  ...
  if(Serial.available() > 0){
    ...
    if (data == 'c') Serial.println("ESC Calibration Mode.");
  ...
  if(data == 0 && new_function_request == false){                                       //Only start the calibration mode at first start. 
    ...
    if (data == 'c') {
    calibrate_esc(); // Call the ESC calibration function.
    data = 0; // Reset the data variable to prevent repeated calibration.
  }
...
void calibrate_esc() {
  Serial.println("ESC Calibration Mode Started.");
  Serial.println("Send high throttle signal (2000µs) to all ESCs.");

  // Send maximum throttle signal (2000µs) to all ESCs
  esc_1 = 2000;
  esc_2 = 2000;
  esc_3 = 2000;
  esc_4 = 2000;
  ...
  delay(8000); // Wait for 8 seconds to allow ESCs to detect high throttle

  Serial.println("Send low throttle signal (1000µs) to all ESCs.");

  // Send minimum throttle signal (1000µs) to all ESCs
  esc_1 = 1000;
  esc_2 = 1000;
  esc_3 = 1000;
  esc_4 = 1000;
  ...
  delay(8000); // Wait for 8 seconds to allow ESCs to detect low throttle
